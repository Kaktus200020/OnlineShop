...
namespace OnlineShop_4M_DataAccess.Repository
{
    public class ProductRepository : Repository<Product>, IProductRepository
    {
        ...
        public IEnumerable<SelectListItem> GetAllDropDownList(string obj)
        {
            ...
            {
                return context.Category.Select(x => new SelectListItem() { Text = x.Name, Value = x.Id.ToString() });
            }
            if (obj ==PathManager.CompanyName)
            {
                return context.Company.Select(x => new SelectListItem() { Text = x.Name, Value = x.Id.ToString() });
            }
            return null;
        }
